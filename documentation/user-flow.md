...
- **Login and flow routing**
  ...
  - **Sign-in email verification and password recovery (web app):** New **User** and **Business** accounts may receive a **confirmation email** after signup/register (non-blocking). Users can still **log in** before verifying; the session exposes **`emailVerified`** so authenticated profile/credentials surfaces can show resend actions when needed. Public pages expose **forgot/reset/confirm** only (no public resend-confirmation page/link). **Forgot password** uses **`/forgot-password`** and redirects to login on success. **Reset** and **confirm** links land on **`/reset-password?token=…`** and **`/confirm-email?token=…`** and run in a minimal recovery layout. Full mechanics (tokens, refresh invalidation after password change) are in [`auth-email-security-flows.md`](./auth-email-security-flows.md) and [`authentication-and-session.md`](./authentication-and-session.md).
  ...
  - **Web app — authenticated route partitions (Phase 2):** The browser **`userId`** / **`businessId`** segments must equal the logged-in session’s **`id`** (Mongo ObjectId string from JWT); guards redirect to the canonical URL if they differ. This is **client routing hygiene** only — authorization remains on the API.
    ...
    - **Legacy `/app`:** Still behind auth; **business** → **`/business/<id>`**; **user** → **`/<userId>/customer`** (not forced through **`/mode`**). **Unknown paths (`*`)** render a dedicated **Not Found** page.
