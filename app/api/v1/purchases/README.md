...
| PATCH | `/api/v1/purchases/:purchaseId/editSupplierGoodFromPurchase` | Updates one item’s quantity/price and **adjusts** inventory delta. **Manager-only** (same roles as close daily report); editor is resolved from session (userId → Employee for the purchase business); body must include `purchaseInventoryItemsId`, `newQuantityPurchased`, `newPurchasePrice`, and non-empty `reason`. Writes audit fields on the line: `lastEditByEmployeeId`, `lastEditReason`, `lastEditDate`, `lastEditOriginalQuantity`, `lastEditOriginalPrice`. Transactional. |
...
- **Edit:** Body **`purchaseInventoryItemsId`, `newQuantityPurchased`, `newPurchasePrice`, `reason`** (all required). **Manager-only:** identity from session (userId); resolve `Employee` by `{ userId, businessId }` for the purchase, require `currentShiftRole` in (General Manager, Manager, Assistant Manager, MoD, Admin, Supervisor); no on-duty requirement. Set the line’s quantity and price; **$set** on the line `lastEditByEmployeeId`, `lastEditReason` (trimmed), `lastEditDate` (new Date()), **`lastEditOriginalQuantity`**, **`lastEditOriginalPrice`** for audit. Adjust `totalAmount` by the price delta and update inventory by the quantity delta. Transaction.
...
- **Purchase item (subdocument):** `supplierGoodId`, `quantityPurchased`, `purchasePrice`, optional audit fields **`lastEditByEmployeeId`, `lastEditReason`, `lastEditDate`, `lastEditOriginalQuantity`, `lastEditOriginalPrice`** (set when a manager edits the line).
