...
- **Notification inbox:** Each user has an optional `notifications[]` array: `{ notificationId, readFlag, deletedFlag }`. When a notification is created or updated with **customersRecipientsIds**, the notifications API pushes this entry into each recipient User’s `notifications` array. Users also receive **order confirmation** notifications (and an email receipt) after paying for a self-order or delivery; the customer can show the notification or email to staff when collecting the order. Users **do not** create or delete notification documents; they only update their own **inbox state** (read/deleted flags) via the sub-routes under `users/[userId]/`.
...
**Implementation note:** The folder is `[userId]`, so the dynamic param is `params.userId`. The handler in `markNotificationAsDeleted` should use `context.params.userId` (not customerId) to match the route segment. Across the app, **session.user.id** (userId) is the single identity used for attribution (orders, sales instances, daily reports); no request bodies send employeeId or customerId for “who am I.”
