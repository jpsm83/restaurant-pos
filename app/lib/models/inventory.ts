...
const inventoryCountSchema = new Schema(
  {
    countedDate: { type: Date, default: Date.now }, // Date when the count was performed

    // ************************* IMPORTANT *************************
    // this quantity is base on the supplierGood.MEAUREMENTUNIT
    currentCountQuantity: {
      type: Number,
      required: [true, "Current count quantity is required!"],
    }, // quantity of the good in the current real count
    // *************************************************************
    ...
    deviationPercent: { type: Number, default: 0 }, // differece between the dynamicSystemCount and the currentCountQuantity in percentage. For a perfect inventory, this number should be 0
    lastCount: { type: Boolean, default: true }, // if this is the last count of the month, only this one can be updated
    ...
    reedited: {
      reeditedByEmployeeId: {
        type: Schema.Types.ObjectId,
        ref: "Employee",
        index: true, // indexing references is a performance optimization, speed queries that frequently filter by this field
      }, // Employee who re-edited - employee in session
      date: { type: Date }, // Date when the re-edit occurred
      reason: {
        type: String,
        required: [true, "Reedited reason is required!"],
      }, // Reason for the re-edit
      originalValues: {
        currentCountQuantity: {
          type: Number,
...
const inventoryGoodsSchema = new Schema(
  {
    ...
    monthlyCounts: { type: [inventoryCountSchema], default: undefined }, // Array of count events objects for the month
...
const inventorySchema = new Schema(
  {
    ...
    setFinalCount: {
      type: Boolean,
      required: [true, "Set final count is required!"],
      default: false,
    }, // Locking mechanism to prevent further counts, is auto set to true on the final of the last day of the month
